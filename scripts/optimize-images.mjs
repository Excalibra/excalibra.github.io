/**
 * Batch optimize images in the site (content / static / assets)
 * - Longest edge > 1600 → scale down proportionally
 * - Only process if size exceeds threshold; overwrite only if result is significantly smaller
 * - Do not change extensions, do not enter themes/
 *
 * Usage:
 *   pnpm install
...
const MAX_EDGE = 1600;
const MIN_BYTES = 80 * 1024; // Skip files smaller than this
const JPEG_QUALITY = 80;
const WEBP_QUALITY = 78;
const MIN_SAVE_RATIO = 0.05; // Overwrite only if at least 5% is saved
...
async function optimizeOne(file) {
  ...
  // Skip if not large and not obviously compressible: small size and < 200KB
  ...
  img = img.rotate(); // Respect EXIF orientation
